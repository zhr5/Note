...
![在这里插入图片描述](images/distributed-transaction/20210710220330858.png)
...
![img](images/distributed-transaction/20200811153200968.png)

#### **幻读（**Phantom Read**）**

事务A进行范围查询时，事务B中新增了满足该范围条件的记录，当事务A再次按该条件进行范围查询，会查到在事务B中提交的新的满足条件的记录（**幻行** Phantom Row）。

![img](images/distributed-transaction/20200811155234313.png)

#### **不可重复读（U**nrepeatable Read**）**

事务A在读取某些数据后，再次读取该数据，发现读出的该数据已经在事务B中发生了变更或删除。

![img](images/distributed-transaction/20200811155249156.png)
...
![img](images/distributed-transaction/20200811145639223.png)
...
![在这里插入图片描述](images/distributed-transaction/20210708215728581.png)
...
![在这里插入图片描述](images/distributed-transaction/20210708215843901.png)
...
下图演示了一个服务同时操作2个库的情况：
![在这里插入图片描述](images/distributed-transaction/20201102225133982.jpg)
...
![在这里插入图片描述](images/distributed-transaction/20201102225150285.jpg)
...
![在这里插入图片描述](images/distributed-transaction/20201102225213956.jpg)
...
![在这里插入图片描述](images/distributed-transaction/2020110223095013.jpg)
...
![在这里插入图片描述](images/distributed-transaction/20210709140731779.png)
...
![在这里插入图片描述](images/distributed-transaction/20210708142634430.png)
...
![img](images/distributed-transaction/20200726001643793.png)



### XA协议的实现

![在这里插入图片描述](images/distributed-transaction/20210709141625435.png)
...
![1625812180865](images/distributed-transaction/20210711205210744.png)
...
![事务提交示意图](images/distributed-transaction/692462-20190407131001597-1193625164.png)
...
![事务中断示意图](images/distributed-transaction/692462-20190407131017854-1201369651.png)
...
![img](images/distributed-transaction/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2t1c2VkZXhpbmdmdQ==,size_16,color_FFFFFF,t_70.png)
...
![在这里插入图片描述](images/distributed-transaction/20201102225454976.jpg)
...
![三阶段提交示意图](images/distributed-transaction/692462-20190407142856081-1418301502.png)
...
![img](images/distributed-transaction/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2t1c2VkZXhpbmdmdQ==,size_16,color_FFFFFF,t_70-1680708075355319.png)
...
![在这里插入图片描述](images/distributed-transaction/20210709231218519.png)
...
![在这里插入图片描述](images/distributed-transaction/20210710003838193.png)
...
![在这里插入图片描述](images/distributed-transaction/20210710152940406.png)
...
![在这里插入图片描述](images/distributed-transaction/20210710003809243.png)

在rocketMq中，不论是producer收到broker存储半消息成功返回后执行本地事务，还是broker向producer反查消息状态，都是通过**回调机制**完成，我把producer端的代码贴出来你就明白了：

![img](images/distributed-transaction/20200512221348392.jpg)
...
![在这里插入图片描述](images/distributed-transaction/20210710001626337.png)
...
![img](images/distributed-transaction/20200825135927900.png)
...
![在这里插入图片描述](images/distributed-transaction/20210709233029528.png)
...
![img](images/distributed-transaction/20210710005654223.png)
...
![在这里插入图片描述](images/distributed-transaction/20210710010954827.png)
...
消息中间件在分布式系统中的核心作用就是异步通讯、应用解耦和并发缓冲（也叫作流量削峰）。在分布式环境下，需要通过网络进行通讯，就引入了数据传输的不确定性，也就是CAP理论中的分区容错性。
 ...
 ![1233356-44395b81aed884dd.png](https://ss.csdn.net/p?https://upload-images.jianshu.io/upload_images/1233356-44395b81aed884dd.png)

消息发送一致性是指**产生消息的业务动作与消息发送动作一致**，也就是说如果业务操作成功，那么由这个业务操作所产生的消息一定要发送出去，否则就丢失。

#### 常规MQ消息处理流程和特点

 ![1233356-5f396e612460cc4c.png](https://ss.csdn.net/p?https://upload-images.jianshu.io/upload_images/1233356-5f396e612460cc4c.png)

常规的MQ队列处理流程无法实现消息的一致性。所以，需要借助半消息、本地消息表，保障一致性。

### 消息重复发送问题和业务接口幂等性设计

 ![1233356-7f06f451b8bfdb8c.png](https://ss.csdn.net/p?https://upload-images.jianshu.io/upload_images/1233356-7f06f451b8bfdb8c.png)
...
![在这里插入图片描述](images/distributed-transaction/20210710092138838.png)
...
![在这里插入图片描述](images/distributed-transaction/20190815161155716.png)
**TCC 分布式事务模型包括三部分：**

![image.png](images/distributed-transaction/thumb.png)
...
![img](images/distributed-transaction/20191210222510796.png)
...
![img](images/distributed-transaction/20191210222510661.png)
...
![在这里插入图片描述](images/distributed-transaction/20201102225620490.jpg)
...
![在这里插入图片描述](images/distributed-transaction/20210711191436977.png)
...
![img](images/distributed-transaction/d1160924ab18972bb076b030d40ebe8f9c510ac1.png)半消息
...
![img](images/distributed-transaction/241f95cad1c8a7864fa441a156ca0c3b72cf50d8.png)本地消息表
...
![640?wx_fmt=png](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_png/nibOZpaQKw09icwew0IKibawDpFiaiaoQyVnZib6tic9rNAicNn6I6LhuSHs31Kww2jJv1YibCvbkUqN0OzZYk4b17TytyQ/640?wx_fmt=png)
...
![在这里插入图片描述](images/distributed-transaction/20210710161530943.png)

Seata的TC、TM、RM三个角色 ， 是不是和XA模型很像. 下图是XA模型的事务大致流程。

![在这里插入图片描述](images/distributed-transaction/20210710161627404.png)
...
![640?wx_fmt=jpeg](https://ss.csdn.net/p?https://mmbiz.qpic.cn/mmbiz_jpg/nibOZpaQKw09icwew0IKibawDpFiaiaoQyVnZpWibNo9daxIshiaduyW2AwqkFx9dXEPNicyKwMicZdg41aStokBZhB0E8Q/640?wx_fmt=jpeg)
...
![img](images/distributed-transaction/20201009172509465.png)
...
![在这里插入图片描述](images/distributed-transaction/20191117211026990.png)
...
![img](images/distributed-transaction/20210711210955548.png)

在订单服务的数据库中，存在如下的订单表 t_order：

![img](images/distributed-transaction/20210711211013809.png)

现在，id 为 40002 的用户要购买一只商品代码为 20002 的鼠标，整个分布式事务的内容为：

1）在库存服务的库存表中将记录

![img](images/distributed-transaction/20210711211030835.png)

修改为

![img](images/distributed-transaction/20210711211044143.png)

2）在订单服务的订单表中添加一条记录

![img](images/distributed-transaction/20210711211102524.png)

以上操作，在 AT 模式的第一阶段的流程图如下：

![img](images/distributed-transaction/20210711211149471.png)

![在这里插入图片描述](images/distributed-transaction/20210711211222402.png)
...
![img](images/distributed-transaction/20210711211250507.png)
...
![img](images/distributed-transaction/20210711211745801.png)
...
![img](images/distributed-transaction/20210711211811749.png)
...
![在这里插入图片描述](images/distributed-transaction/20210710164507421.png)

tx1 二阶段全局提交，释放 **全局锁** 。tx2 拿到 **全局锁** 提交本地事务。

如果 tx1 的二阶段全局回滚，则 tx1 需要重新获取该数据的本地锁，进行反向补偿的更新操作，实现分支的回滚。

![在这里插入图片描述](images/distributed-transaction/2021071016463052.png)
...
![在这里插入图片描述](images/distributed-transaction/20210710165141349.png)
...
- **TCC 性能更高**
  ...
  ![a](images/distributed-transaction/ecaeda572495e4ea5e308fb938a38fd5.png)
...
![在这里插入图片描述](images/distributed-transaction/20210710224912234.png)
...
![a](images/distributed-transaction/6223e8be682d613f39e5a27587635f73.png)

假如用户购买 100 元商品，要扣减 100 元。

TCC 事务首先对这100元的扣减金额进行预留，或者说是先冻结这100元：

![a](images/distributed-transaction/29586d0d5b4a08e27f784a80a450e9a5.png)
...
![a](images/distributed-transaction/6227e7939dc860283ea799abcdccbe2b.png)

#### 第二阶段 Cancel

如果全局事务回滚，就把冻结的金额进行解冻，恢复到以前的状态，TC 会控制当前分支事务回滚，如果回滚失败，TC 会反复尝试，直到回滚完成为止。

![a](images/distributed-transaction/ea3d9416a1db9998241c160b67078c51.png)

#### 多个事务并发的情况

多个TCC全局事务允许并发，它们执行扣减金额时，只需要冻结各自的金额即可：

![a](images/distributed-transaction/5c78e69b171903aaef07621076019fba.png)
...
![Saga模式示意图](images/distributed-transaction/TB1Y2kuw7T2gK0jSZFkXXcIQFXa-445-444.png)
...
![示例状态图](images/distributed-transaction/demo_statelang.png)
...
![img](images/distributed-transaction/TB1hSpccIVl614jSZKPXXaGjpXa-1330-924.png)
...
![xa-fw](images/distributed-transaction/TB1uM2OaSslXu8jSZFuXXXg7FXa-1330-958.png)
...
![ds1](images/distributed-transaction/TB11_LJcggP7K4jSZFqXXamhVXa-1564-894.png)
...
![ds2](images/distributed-transaction/TB1qJ57XZieb18jSZFvXXaI3FXa-1564-894.png)
...
![在这里插入图片描述](images/distributed-transaction/20210711130223963.png)
...
![在这里插入图片描述](images/distributed-transaction/20210711130524193.png)
...
![在这里插入图片描述](images/distributed-transaction/20210711130854311.png)
